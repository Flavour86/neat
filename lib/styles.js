...
function styles(pathFile){
	...
	if (pathFile && typeof pathFile === 'string') {
		...
		if ((dir.indexOf(conf.folders.css + '/' + conf.folders.cssCommon) > -1 && dir.indexOf(conf.folders.cssVariable) === -1) || dir.indexOf(conf.folders.component) > -1) {
			source = path.join(c_paths.src, conf.folders.css, 'common.css')
			dest = path.join(c_paths.tmp, conf.folders.css)
		} else if (dir.indexOf(c_paths.src + '/' + conf.folders.cssLib) < 0) {
			source = [
